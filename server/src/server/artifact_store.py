...
class ArtifactStore:
    ...
    def seed_artifacts(self, artifacts: tuple[Artifact, ...]) -> None:
        with self.engine.begin() as connection:
            for artifact in artifacts:
                artifact_insert = sqlite_insert(artifacts_table).values(artifact_row_values(artifact))
                connection.execute(
                    artifact_insert.on_conflict_do_update(
                        index_elements=[artifacts_table.c.id],
                        set_=artifact_update_values(artifact_insert),
                    )
                )
                replace_artifact_tags(connection, artifact)
    ...
    def search_artifacts(
        ...
        conditions = artifact_search_conditions(kind, object_type, placement, tag, query)
    ...
    def get_artifacts_by_ids(self, artifact_ids: list[str]) -> ArtifactBatchResult:
        unique_ids = unique_in_order(artifact_ids)
        with self.engine.connect() as connection:
    ...
    def load_artifacts_by_ids(self, connection: Connection, artifact_ids: list[str]) -> list[Artifact]:
        ...
        tags_by_artifact_id = load_artifact_tags_by_id(connection, artifact_ids)
        ...
        for artifact_id in artifact_ids:
            ...
            artifacts.append(read_artifact_row(row, tags_by_artifact_id.get(artifact_id, [])))

        return artifacts


def artifact_row_values(artifact: Artifact) -> RowValues:
...
def artifact_update_values(artifact_insert: Any) -> RowValues:
    return {
...
def replace_artifact_tags(connection: Connection, artifact: Artifact) -> None:
...
def artifact_search_conditions(
    kind: str | None,
...
def load_artifact_tags_by_id(connection: Connection, artifact_ids: list[str]) -> dict[str, list[str]]:
...
def read_artifact_row(row: Mapping[str, Any], tags: list[str]) -> Artifact:
    return Artifact(
        id=str(row["id"]),
        kind=str(row["kind"]),
